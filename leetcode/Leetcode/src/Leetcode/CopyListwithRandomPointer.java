...
public class CopyListwithRandomPointer {
	public static void main(String[] args) {
		RandomListNode one = new RandomListNode(5);
		RandomListNode two = new RandomListNode(4);
		one.next = two;
		RandomListNode three = new RandomListNode(3);
		two.next = three;
		RandomListNode four = new RandomListNode(2);
		three.next = four;
		RandomListNode five = new RandomListNode(1);
		four.next = five;
		//five.next = one;
		System.out.print(copyRandomList_2(one));
	}
	public RandomListNode copyRandomList_5(RandomListNode head) {
		RandomListNode cur = new RandomListNode(-1);
		RandomListNode cp = new RandomListNode(-1);
		cur.next = head;
		while (cur != null) {
			cp.next = new RandomListNode(cur.label);
			cp = cp.next;
			cur = cur.next;
		}
		return cp.next;
